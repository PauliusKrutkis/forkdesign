...
/** Per-model timing for one attempt inside a fix run's fallback chain. */
...
export interface FixRunLogEntry {
  ...
  /** Total variants requested for this Fix run. */
  variantCount?: number;
  /** 1-based index when generating multiple independent variants. */
  variantIndex?: number;
}

function fixLogPath(projectRoot: string): string {
  return path.join(projectRoot, "designs", "logs", "fix-runs.jsonl");
}

/**
 * Append a fix-run record as one JSONL line. Best-effort: logging must never
 * break or slow down the fix itself, so failures are swallowed with a warning.
 */
export async function appendFixRunLog(
  projectRoot: string,
  entry: FixRunLogEntry
): Promise<void> {
  try {
    const file = fixLogPath(projectRoot);
    await mkdir(path.dirname(file), { recursive: true });
    await appendFile(file, `${JSON.stringify(entry)}\n`, "utf8");
  } catch (err) {
    console.warn(
      `[fix-log] failed to write run log: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
